<template>
    ...
    <AppLayout>
        <template #header>
            <div class="flex justify-between">
                ...
                <div class="flex">
                    ...
                    <Link v-if="$page.props.auth.user.permissions.includes('write products')" class="button1 mb-2 py-2 px-3 bg-blue-300 shadow border-gray-300 border hover:bg-blue-400 rounded mr-3" as="button" href="/products/create">Create Product</Link>
                    <a href="/products/pdf" class="flex  mb-2 py-2 px-3 bg-gray-300 shadow border-gray-300 border hover:bg-gray-400 rounded mr-3 " target="blank">
        ...
        <div class="py-12">
            <div class="max-w-7xl mx-auto sm:px-6 lg:px-8 flex flex-wrap"  v-if="$page.props.auth.user.permissions.includes('read products')" >
                <ProductCard v-for="product in props.products" :product="product" :key="product.id"/>
...
function search(ev){
    router.visit('/products/search/'+ ev.target.value);
